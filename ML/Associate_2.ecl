...
//o_v Dataset represents the vertical form of incoming basket values.
//o_c Dataset represents the repition count of each basket in o_v.


EXPORT Associate_2(DATASET(Types.ItemElement) o_v,DATASET(Types.ItemElement) o_c, Types.t_Count M) := MODULE
...
//
SHARED D := o_v;
joined_layout := { RECORDOF(o_v) OR {Types.t_count support} };
joined_layout makeJoin(RECORDOF(o_v) L, RECORDOF(o_c) R) := TRANSFORM
    SELF.support := R.value;
    SELF := L;
END;
SHARED countsLookUpTable := JOIN(o_v, o_c, LEFT.id = RIGHT.id, makeJoin(LEFT, RIGHT), SMART);
...
EXPORT Apriori1 := FUNCTION
	/*
	R := RECORD
	  countsLookUpTable.value;
          Types.t_Count support := countsLookupTable.repetition;
	END;
	RETURN TABLE(countsLookUpTable, r, value, repetition, MERGE)(support>=M);
	*/
	result := countsLookUpTable;
	RETURN result;
  END;

EXPORT Apriori1_v2 := FUNCTION
	R := RECORD
	  countsLookUpTable.value;
          Types.t_Count support := SUM(GROUP, countsLookupTable.support);
	END;
	RETURN TABLE(countsLookUpTable, r, value, MERGE)(support>=M);
END;
...
//	EC1 := IF ( COUNT(Apriori1) < Config.MaxLookup/SIZEOF(Apriori1), dsl, dbl );
	...
	EPE convert(RECORDOF(Apriori1) R) := TRANSFORM
		SELF.support := R.support;
		SELF.pat := Str.cleanspaces((STRING)R.value);
		SELF.id := R.id;
	END;
	EC1 := PROJECT(Apriori1, convert(LEFT));
	...
	Groups(DATASET(EPE) di) := PROJECT(TABLE(di,{Pat,Types.t_Count Support := SUM(GROUP, support)},Pat, MERGE)(Support>=M),TRANSFORM(EPE,SELF.id := 0,SELF:=LEFT));
	...
	//EC1_R_temp := PROJECT(EC1, TRANSFORM(RECORDOF(EC1), SELF.id := 0; SELF := LEFT));
	//EC1_R := EC1 + EC1_R;
	//EC1_R := EC1;
...
// For every pattern in P	break it out into rule records
	...
	RuleElement NoteSig(N Le,Apriori1_v2 ri) := TRANSFORM
	  SELF.sig := Le.Conf * Records / 100 / ri.Support;
		SELF := le;
	END;
	WithSig := JOIN(WithConf,Apriori1_v2,LEFT.Next=RIGHT.Value,NoteSig(LEFT,RIGHT));
	//RETURN WithSig;
	RETURN WithSig;
END;
